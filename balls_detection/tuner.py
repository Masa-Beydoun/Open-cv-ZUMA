...
def tune_parameters(screenshot_path):
    ...
    while True:
        ...
        if cv2.waitKey(1) & 0xFF == ord("q"):
            print(
                f"Final Values -> dp={dp}, minDist={min_dist}, p1={p1}, p2={p2}, minR={min_r}, maxR={max_r}"
            )
            break
...
class ZumaBot:
    ...
    def detect_from_frame(self, frame):
        ...
        if circles is not None:
            circles = np.round(circles[0, :]).astype("int")
            for x, y, r in circles:
                if y - r < 0 or x - r < 0 or y + r > h_curr or x + r > w_curr:
                    continue
...
def select_game_region():
    ...
    with mss.mss() as sct:
        ...
        r = cv2.selectROI(
            "Select Game Region", screenshot, showCrosshair=True, fromCenter=False
        )
...
if __name__ == "__main__":
    ...
    tune_parameters('balls_detection\c.png')
