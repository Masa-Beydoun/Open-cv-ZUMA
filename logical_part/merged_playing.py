...
# ============================================================
class BallTracker:
    def __init__(self, max_distance=TRACKING_MAX_DISTANCE, max_frames_missing=TRACKING_MAX_MISSING_FRAMES):
        self.tracked_balls = []
        self.next_id = 0
    ...
    def update(self, detected_balls):
        ...
        return [b["original_ball"] for b in self.tracked_balls if b["confidence"] >= 0.4]
...
def active_color_check(sct, monitor, initial_frog_box, game_offset, color_config):
    ...
    try:
        ...
        if best_match == "Green":
            if DEBUG_MODE:
                log(f"Rejected Green. Hue: {hue:.1f}", "DEBUG")
...
def check_if_shootable(target_ball, all_balls, frog_center):
    ...
    blocked, _ = is_shot_blocked(frog_center, target_ball["position"], all_balls, target_ball)
    return not blocked
...
def find_dump_zone(balls, frog_center, game_width, game_height):
    ...
    for zone_x, zone_y in candidate_zones:
        ...
        if min_distance_to_ball > max_min_distance and min_distance_to_ball > DUMP_ZONE_MIN_DISTANCE:
...
def find_best_target(balls, current_ball_color, frog_center=None):
    ...
    max_distance = max(b.get("distance", 0) for b in shootable_balls) if shootable_balls else 1
...
#     تنفيذ التصويب
# ============================================================
def execute_shot(target_pos, game_offset, target_color, score, reason, frog_center=None, is_dump=False):
...
def print_shot_history():
    ...
    for shot in shot_history[-20:]:
        ...
        print(f"{shot_type} #{shot['id']:3} | {shot['time']} | {shot['color']:8} | Score: {shot['score']:.0f}")
    ...
    with mss.mss() as sct:
        ...
        while RUNNING:
            ...
            if PAUSED:
                ...
                cv2.putText(blank_screen, "PAUSED", (150, 150),
            ...
            if capture_area:
                try:
                    ...
                    if CURRENT_BALL_COLOR is None and frog_center:
                        detected_color, sample_pt, sample_rad = active_color_check(
                            sct, full_monitor, frog_box, (game_x, game_y),
                    ...
                    if AUTO_SHOOT and CURRENT_BALL_COLOR:
                        ...
                        if time_since_last > SHOT_COOLDOWN:
                            if frog_center and stable_balls:
                                ...
                                if should_dump_ball(score, CURRENT_BALL_COLOR, detected_balls):
                                    dump_zone = find_dump_zone(stable_balls, frog_center, 
                                                               frog_detector.w, frog_detector.h)
                                    
                                    if dump_zone:
                                        cv2.circle(result, dump_zone, 30, (0, 255, 255), 3)
                                        cv2.line(result, frog_center, dump_zone, (0, 255, 255), 2)
                                        
                                        game_offset = (game_x, game_y)
                                        execute_shot(dump_zone, game_offset, CURRENT_BALL_COLOR, 
                                                   score, "DUMP", frog_center, is_dump=True)
                                ...
                                if best_target:
                                    ...
                                    blocked, _ = is_shot_blocked(frog_center, target_pos, stable_balls, best_target)
                                    line_color = (0, 0, 255) if blocked else (0, 255, 0)
                                    
                                    cv2.line(result, frog_center, target_pos, line_color, 3)
                                    cv2.circle(result, target_pos, 15, (255, 0, 255), 3)
                                    
                                    game_offset = (game_x, game_y)
                                    execute_shot(target_pos, game_offset, best_target["color"], 
                                               score, reason, frog_center=frog_center)
                                    last_shot_time = current_time
                    ...
                    if last_sample_info and game_x > 0:
                        ...
                        cv2.circle(result, (local_sx, local_sy), s_radius + 2, (0, 255, 255), 2)
                    ...
                    cv2.putText(result, f"Frog: {CURRENT_BALL_COLOR}",
                        (10, 55), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 200, 255), 1)
                    
                    status = "ON" if AUTO_SHOOT else "OFF"
                    color = (0, 255, 0) if AUTO_SHOOT else (0, 0, 255)
                    cv2.putText(result, f"[S] {status}", (10, 90), cv2.FONT_HERSHEY_SIMPLEX, 0.6, color, 2)
                ...
                blank_screen = np.zeros((300, 500, 3), dtype=np.uint8)
                cv2.putText(blank_screen, "Searching...", (100, 150),
